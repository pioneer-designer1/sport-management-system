...
sidebar();
 ...
 if(isset($_POST) && !empty($_POST)){
			...
			}else
			{
					...
					$_FIELDS	=	array(
											"nameofsport"			=>	array("text",1,"Name Of Sport"),
											"parent"					=>	array("int",0,"Parent Sport"),
											"descriptionofsport"	=>	array("text",0,"Description Of Sport"),
											"id"						=>	array("text",0,"ID")
											);
...
$sql	=	sprintf("UPDATE`sport_mstr` SET `sport` = '%s',`parent`='%s',`description`='%s' WHERE `id`='%s'",
												$out['nameofsport'],
												$out['parent'],
												$out['descriptionofsport'],
												$out['id']
												);
				...
				if(!$return){
					...
					$id		=	 $out['id'];
					$update	=	1;
					$_SESSION['message'][]	=	"Sport [".$out['nameofsport']."] Updated Successful with ID = $id";
				}
...
if(isset($_GET['id']) && !empty($_GET))
{
	...
	if(isset($out['error'])){
		...
		$db_object->get_single_sport_record($out['id']);
...
if(($update==1) && !(isset($_GET['id'])) || $error==1){
	?>
   <a href="sport_manager.php">Click To See Listing </a>
   <?php
	exit();
	}
?>
 ...
 <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
  <div class="form-group">
    <label for="nameofsport">Name Of Sports:</label>
<input type="text" class="form-control" value="<?php echo $row['sport']?>" id="nameofsport" name="nameofsport" placeholder="Name Of Sport here...">
  </div>
<?php 
	$db_object->get_all_active_parent_sports(); 
...
?>
  ...
  <div class="form-group">
      ...
      <textarea name="descriptionofsport" class="form-control"><?php echo $row['description']; ?></textarea>
