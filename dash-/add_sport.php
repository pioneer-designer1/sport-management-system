 ...
 if(isset($_POST) && !empty($_POST)){
			...
			}else
			{
				...
				if(isset($out['error'])){
						...
						$sql	=	sprintf("INSERT INTO `sport_mstr` (`sport`,`parent`,`description`)
												VALUES ('%s','%s','%s')",
												$out['nameofsport'],
												$out['parent'],
				...
				if(!$return){
					...
					$id		=	 mysql_insert_id();
					$_SESSION['message'][]	=	"Sport [".$out['nameofsport']."] Added Successful with ID = $id";
					$add=1;
				}
...
?>
 <h3 class="text-center">Add Sport</h3>

 <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
  <div class="form-group">
    <label for="nameofsport">Name Of Sports:</label>
   	 <input type="text" class="form-control" id="nameofsport" name="nameofsport" placeholder="Name Of Sport here...">
  </div>
<?php 
...
?>
  <div class="form-group">
    ...
    <select name="parent" class="form-control" >
			...
			do{
					?>
               <option value="<?php echo $row['id'];?>"><?php echo $row['sport'];?></option>
               <?php
  ...
  <div class="form-group">
      ...
      <textarea name="descriptionofsport" class="form-control"></textarea>
